...
- **Fix pattern** — at the workflow level: one automatic retry in `.github/workflows/ci-linux.yml`. At the source level: use the existing `_direntName` / `_direntNameLength` helpers in `ForSwiftFoundationOnly.h` (the same pattern PR #4892 applied to `FileManager+POSIX.swift`). For deterministic-failure runs (both attempts fail), the only short-term mitigation is reducing test parallelism on Linux — but this is rare in practice.
