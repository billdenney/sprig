...
5. **Accidental secret commits.** A file with API keys / private keys gets staged. Mitigations: **gitleaks-style staged-secret scan — shipped (ADR 0092), promoted from M6** as the default-on `stagedSecretDetected` pre-flight rail (`GitCore.SecretScan`: vendored regex + entropy, no bundled binary; warn-and-proceed; `.sprig/secret-allow` allowlist); `.gitignore` warnings on names like `.env` / `*.pem` / `id_rsa` (ADR 0070/0075); push-time secret scan over `@{u}..HEAD` (ADR 0093); recovery wizard for "remove file from history" that emphasizes revocation-first (still pending).
