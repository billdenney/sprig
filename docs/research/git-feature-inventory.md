...
- `LFSKit.DocumentStoreHeuristic.evaluate(runner:)` (the one-time, repo-level "this looks like a document store — set up LFS?" offer): `git ls-files -z` for the tracked path list, then a pure count-share classification (curated-binary files ≥ 50% of tracked files **and** ≥ 5 tracked files → offer; both injectable). No per-file stat, no history walk — the path list is the sample. Output is a provider-neutral `DocumentStoreOffer`; consent routes each ranked `*.ext` pattern to `LFSTrack.track`.
- `LFSKit.DocumentStoreOfferFlag` (once-per-repo suppression for the offer): `git rev-parse --path-format=absolute --git-common-dir` resolves the shared (worktree-safe, repo-global) git dir; the flag is a plain marker file at `<common-dir>/sprig/document-store-offer-made` — **not** a ref (a ref would be fetched/pushed and shared; "have we offered on this clone" is local state). `hasOffered`/`recordOffered` are file-existence/atomic-write under that dir.
