...
- [x] Benchmarks pass on a synthesized 100k-file fixture within ADR 0021 budgets (CPU, RAM, status latency). **Hosted-CI smoke covers the wall-clock half**: `tests/integration/Tests/IntegrationTests/Status100kFileBudgetTests.swift` synthesizes a 100 000-file repo on every PR (macOS / Linux / Windows), runs `git status --porcelain=v2 -z` + `PorcelainV2Parser.parse(_:)` against it, and asserts the measured window completes inside a 60-second wall-clock ceiling. The benchmark ladder in `Benchmarks/SprigCoreBenchmarks.swift` is also extended to 100k so the **dedicated perf gates** (sustained CPU%, peak RSS, fine-grained latency comparisons) run automatically when the self-hosted runner comes online — see `docs/ci/self-hosted.md`. ADR 0021's full statistical budgets still need self-hosted hardware for stable measurement; the integration smoke catches catastrophic regressions in the meantime.
...
All three M1 → M2 gates are now wired into hosted CI on every PR via the `ci/m1-m2-exit-gate-integration-tests` branch. ADR 0021's fine-grained perf comparisons (sustained CPU%, peak RSS, latency distributions) still wait on the self-hosted runner — that part of the budget validation lives in `.github/workflows/benchmarks.yml` and goes nightly-green when the runner is provisioned.
