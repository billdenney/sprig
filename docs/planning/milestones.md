...
- [ ] Benchmarks pass on a synthesized 100k-file fixture within ADR 0021 budgets (CPU, RAM, status latency). **Deferred** to self-hosted runner provisioning — see `docs/ci/self-hosted.md`. The benchmarks workflow (`.github/workflows/benchmarks.yml`) targets `[self-hosted, macOS, arm64]` and goes nightly-green once the runner is online.
...
The 100k-file benchmark is M1's last open gate; the two parser-fidelity + event-budget gates above were wired into hosted CI in branch `ci/m1-m2-exit-gate-integration-tests` and are now enforced on every PR.
