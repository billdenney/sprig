...
- **Where:** `PreferencesViewModelTests` "save() writes JSON, subsequent load() reads it back" and `MergeConflictResolverPerRegionTests` "text(regions:) with [.ours, .theirs] splices each region". `RepoAgentAutoBackupTests` "agent with AutoBackupStartup backs up a dirty tree on its tick" joined the family once (2026-06-11) — on the watch list, same treatment. **`SprigctlAgentTests/socketServesSubscriber` (the two-process UDS e2e) + its Windows named-pipe sibling `pipeServesSubscriber` — REMOVED from the watch list, durably fixed 2026-06-19.** This cousin saw `subscriptionEnded(agent_shutdown)` instead of `badgeChanged` under load — the agent's `--duration` cap expired before the dirty→badge round-trip propagated. Bumping the cap (8 → 25 → 60 s, PRs #156/#162) only raised the floor without closing the race (it flaked again at 60 s on BOTH macos-14 and macos-15 in one run on 2026-06-19). Fixed by decoupling the agent's lifetime from the wall-clock cap: `sprigctl agent --exit-on-last-client` shuts the agent down cleanly (exit 0) on its last client's disconnect, so `--duration` is now a pure safety ceiling (300 s) the round-trip never races. Verified **25/25 on the macOS VM under full CPU saturation, max 2 s/run** (vs the prior 64–65 s racing the cap); ADR 0076 (UDS) / 0067 (named pipe).
