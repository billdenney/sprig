...
- **Likelihood:** 3. All **21** `TaskWindowKit` view models are Swift actors (14 when this risk was filed; the count grew while the gate went unexercised); the documented consumption pattern ("SwiftUI wraps reads in `Task` from `@MainActor`") is plausible but has never been exercised against a real shell. swift-cross-ui's story is even less proven. If the pattern is wrong (e.g. shells want `@Observable` main-actor façades), every VM shares the flaw.
- **Impact:** 3. A pattern rework across **21** VMs mid-M3, or — worse — per-window ad-hoc workarounds that diverge the two shells. Impact grows with every VM added before the gate runs — which is why the count is frozen at 21 until it does.
- **Mitigation:** the M3 **spike-first gate** (`milestones.md`): one real window bound to one existing VM per shell *before* mass window-building; any pattern change lands once, as an ADR, while it's cheap.
- **Rig for the open (runtime) half *(2026-07-27)*:** the June spike showed the runtime question needs an **interactive desktop session** — `SpikeApp.exe` fail-fasts on the headless Windows VM over SSH (session 0, no WindowsAppRuntime), and there is still no Mac. The only interactive desktop available today is the maintainer's **Linux** box, so the runtime half runs there via SwiftCrossUI's **GTK backend**, binding the real `StatusViewModel` by path dependency. This is legitimate despite ADR 0054 putting a Linux *shell* post-1.0: the question under test — does mutating view state from `Task { await actorVM.verb() }` actually propagate, and on which thread — is backend-independent, so answering it on GTK answers it for AppKit and WinUI too. Backend-specific runtime exposure (WinUI packaging, AppKit main-thread rules) stays per-shell.
